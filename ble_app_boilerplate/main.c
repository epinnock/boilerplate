...
#define ASSERT_LED_PIN_NO               23                                   // < Is on when application has asserted. 
*/
//MY DEFINED LEDS
#define IR_LED														21
#define SHOELACE_LED											23

#define DEVICE_NAME                     "KD35 SHOES"                           /**< Name of device. Will be included in the advertising data. */
...
static ble_lbs_t   													 m_lbs;																		 /**< Structure used to identify the LED Service. */
...
{
	/*Original LED output Initialization
    ...
    nrf_gpio_cfg_output(ASSERT_LED_PIN_NO);
	 ...
	 //MY ADDITIONS
	  ...
	  nrf_gpio_cfg_output(SHOELACE_LED);
    nrf_gpio_pin_set(IR_LED);
	  nrf_gpio_pin_set(SHOELACE_LED);
	*/
...
static void services_init(void)
{
	...
	uint32_t       err_code; // Error Code int  
	ble_bas_init_t bas_init; // Battery Service Struct
	ble_lbs_init_t lbs_init; // LED Button Service
    ...
    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&bas_init.battery_level_char_attr_md.read_perm);
	  BLE_GAP_CONN_SEC_MODE_SET_OPEN(&bas_init.battery_level_char_attr_md.write_perm); //Enable Permission to  Write to Battery Level Value
  //BLE_GAP_CONN_SEC_MODE_SET_OPEN(&bas_init.battery_level_char_attr_md.write_perm); Disable Permission to  Write to Battery Level Value
...
int main(void)
{
   ...
   //pwm_config.gpio_num[1]      = 22;
   // pwm_config.gpio_num[2]      = 23;    
